...
//        boolean enabled = loginRequest.isEnabled();
    ...
    @GetMapping("/verify")
    public void verifyAccount(@RequestParam("code") String code, HttpServletResponse response) throws IOException {
        boolean result = userService.verifyAcount(code);
        String verifyUrl = "http://localhost:4200/login";
        if (result) {
            String redirectUrl = response.encodeRedirectURL(verifyUrl);
